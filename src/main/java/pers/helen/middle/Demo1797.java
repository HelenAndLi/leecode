 ...
 * 你需要设计一个包含验证码的验证系统。每一次验证中，用户会收到一个新的验证码，这个验证码在 currentTime 时刻之后 timeToLive 秒过期。如果验证码被更新了，那么它会在 currentTime （可能与之前的
...
public class Demo1797 {
    ...
    public static void main(String[] args){
        ...
        String tokenId = "1a";
        int currentTime = 0;
        Demo1797 obj = new Demo1797(timeToLive);
        obj.generate(tokenId, currentTime);
        obj.renew(tokenId, currentTime);
        int param_3 = obj.countUnexpiredTokens(currentTime);
    }

    public Demo1797(int timeToLive){
